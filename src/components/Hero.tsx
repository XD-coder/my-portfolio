...
const Hero = () => {
  return (
    ...
    >
      ...
      <motion.div
        className="absolute left-1/2 bottom-6 -translate-x-1/2 z-10 flex flex-col items-center"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1.2, duration: 0.7 }}
      >
        <div className="w-7 h-12 rounded-full border-2 border-[var(--color-primary)] flex items-start justify-center relative">
          <motion.div
            className="w-2 h-2 rounded-full bg-[var(--color-primary)] mt-2"
            animate={{ y: [0, 20, 0] }}
            transition={{ repeat: Infinity, duration: 1.5, ease: "easeInOut" }}
          />
        </div>
        <span className="mt-2 text-xs text-[var(--color-muted-foreground)] tracking-wide">Scroll Down</span>
      </motion.div>
    </section>
  );
};
